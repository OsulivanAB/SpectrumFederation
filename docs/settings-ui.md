...
Profile admins can add profile members as admins, remove admins other than the owner, and manage **Linked Characters**. The link dialog asks for Character 1 and Character 2. Unlinking asks for confirmation because shared points, Attendance, and equipment opportunity state stop being combined.
...
Profile admins can choose a character, set that character's persistent specialization, and use Gear Override from a character-equipment board. Empty slots show placeholders; used slots show the item icon, a WoW tooltip, and a red X to clear. Clicking a slot offers loot that fits that slot, plus unresolved awards marked for force assignment. An available slot can be marked consumed when the item is unknown, and that same control clears the consumption later. An already-assigned item can be moved to another empty compatible slot with one REPLACE. Selecting either linked character shows the shared BiS board and award pool. Award ownership stays with the character who received the loot. Manual loot additions enter the pool without consuming an opportunity and keep the selected character and slot. Consumed opportunity — item unknown is visually distinct. Overflow `#278` origins are not listed as associable opportunities. In the item-aware raid equipment popup, an admin slot click opens this page with the character and slot already selected. Final visual behavior still requires Retail QA.
