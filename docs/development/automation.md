...
- CurseForge requires that exact Retail name from the documented Game Versions API at `https://wow.curseforge.com/api/game/versions`. The numeric game-version ID is resolved from that catalog and is not hard-coded. The Retail type comes from `game/version-types`: the author catalog names it `World of Warcraft` / `world-of-warcraft`, and a label that contains `retail` is also accepted. Classic, PTR, public-test, and other non-Retail version types are ignored. A unique patch name is not treated as Retail when that type cannot be identified, including when only a PTR entry exists for an upcoming patch. If the exact Retail patch cannot be identified, CurseForge publishing fails. The catalog result is reused for the rest of that release process.
- CurseForge game-version compatibility is read from `## Interface` in the parent TOC inside the release zip. If the `--interface` argument differs from that packaged value, CurseForge publishing fails instead of claiming the argument's newer patch. Wago still uses the `--interface` argument.
...
- Before uploading, the publisher also checks `GET /api/projects/{projectId}/files` on the CurseForge author API for an exact zip name or display name. A name match is an existing release only when its release type, game version, and usable state also match this publish. A same-named file that is Stable instead of Beta, targets a different game version, or is rejected or unavailable is a visible failure, and the publisher does not upload another copy. That list call is not part of the documented Upload API. A missing, forbidden, or otherwise unusable list response is treated as unavailable and does not block the upload. A listed name that omits those metadata fields is not treated as success; the documented upload endpoint then decides. Duplicate safety still uses the explicit upload response described above. A generic conflict is still a failure.
