...
Each settled Raid Check (`mode == "raid"`) is one presence opportunity, matching the existing point-award boundary: repeated UI runs create additional opportunities. Presence credit is independent of equipment preparedness. Eligible players are the frozen profile roster at run start; present players are the frozen group. A player who was neither on the roster nor in the group is omitted from that opportunity and is not treated as absent. Players with no applicable opportunities display `—`. Do not derive this percentage from `POINT_CHANGE` or Reward Pot `ATTENDANCE_CHANGE` balances.
