...
Identity-wide points and Attendance are sums of retained character logs. Attendance Replay and live in-order fan-out both keep the raw identity total; displayed Attendance clamps to `>= 0`. Unmarked `ARMOR_CHANGE` stays character-local forever; new shared corrections use `scope = "identity"` and `identityMembers`. Identity-scoped `ARMOR_CHANGE` snapshots `preOpAuthorMax` through `LootLog.new` so Replay keeps writer-observed equipment order across same-timestamp authors. Identity-scoped corrections target the identity that existed at write time: Replay applies them only when those `identityMembers` were actually unified in the causal prefix, so a correction written from an incomplete relationship view cannot become active merely because those characters later link. Outsiders joining or leaving do not invalidate a correction while every original `identityMembers` character stays together. Separating any original scope member permanently expires that correction; later relinking the same characters does not resurrect it. A later correction recorded on a newly expanded current identity supersedes earlier overlapping subset-scope state for that slot or ring/trinket family. A later same-slot `USED` does not discard an earlier equal/superset `AVAILABLE`; re-using the displayed opportunity after a clear must not pack historical locals as phantom overflow. Disjoint pre-merge identities still both contribute when they later join: an older subset `AVAILABLE` does not erase another identity's scoped `USED`. A later current-identity correction on a different slot or ring/trinket opportunity does not discard those independent same-slot contributions. Equipment chronology follows `Identity.OrderLogs` causal rank (`preOpAuthorMax`, per-author counters, `sourceLogId`, `sourceLogIds`, `targetAssignmentId`), not the raw `CompareLogs` tie-break. An identity `AVAILABLE` suppresses contributions from that recorded scope only, including that scope's packed overflow for ordinary slots. Singleton identities keep original local Ring1/Ring2 and Trinket1/Trinket2. Linked identities pack currently-active local ring and trinket usages chronologically onto projected Slot 1, then Slot 2, then overflow. When previously independent scoped ring or trinket usages merge, displayed uses pack into opportunity 1 then 2 before overflow instead of preserving source slot indices. Manual identity-scoped clicks still target the displayed projected slot. Item-aware BiS reconstruction lives in `modules/LootHelper/Bis.lua`. It is a derived projection from `SPEC_CHANGE`, `RC_LOOT_COUNCIL`, `BIS_OUTCOME`, `BIS_OVERRIDE`, `MANUAL_AWARD`, and `MANUAL_AWARD_REVERSE`. Frozen `BIS_OUTCOME OVERFLOW` never backfills. Automatic `ASSIGNED` outcomes replay frozen award-time classification (`equipLoc`, item class/subclass, family, slots) rather than current `SpecWeapons` tables. Gear Override is the admin correction UI. Local `BIS_OVERRIDE` ASSIGN/REPLACE mutators check the same occupancy Replay will use (projected BiS cells plus `#278` `GetIdentityArmor`) and do not append a knowingly ineffective correction; REPLACE does not treat `#278` consumption as vacated just because the target assignment currently overlays it. `LootLog.ValidateTable` applies the existing semantic validators to known protocol-3 item-aware types (`SPEC_CHANGE`, `BIS_OUTCOME`, `BIS_OVERRIDE`, `MANUAL_AWARD`, `MANUAL_AWARD_REVERSE`) at the import boundary; unknown future event types stay forward-compatible.

`LootProfile` owns a cached identity projection. Rebuild it when identity-affecting history changes. Profile helpers such as `GetIdentityMembers`, `AreSameIdentity`, `GetIdentityPoints`, `GetIdentityAttendance`, and `GetIdentityArmor` read that cache. Live point and Attendance writes may fan out across the current identity instead of replaying all logs. Full replay remains the convergence path for relationship changes, migration, import, and sync rebuild. Automatic BiS combat assignment keeps class proficiency, spec combat-weapon subclass, and slot occupancy flags distinct. Frozen `BIS_OUTCOME ASSIGNED` rows must stay source-bound to the immutable RC item (`itemString` and `equipLoc` when present) without consulting later `GetItemInfoInstant` or `SpecWeapons` tables. During an active session, automatic `BIS_OUTCOME` writes are coordinator-authoritative, wait for admin-convergence log repairs on start and takeover, and stop once a source-consistent outcome exists for that `awardKey`. Live `BIS_OUTCOME` `NEW_LOG` traffic from anyone except the coordinator is ignored. If that ignored row sits above the local counter prefix, the missing earlier counters are still requested. Repair `AUTH_LOGS` drops a current-session non-coordinator `BIS_OUTCOME` (timestamp at or after `coordEpoch`) and remembers the counter, id, and fingerprint so gap repair and exact-window proof can finish even when other rows in that window were kept; earlier rows and direct snapshot import stay stored. That memory is not advertised. Bulk import still retains historical duplicates. Loot Logs shows the replay `outcomeWinner` under the RC Loot Council category and omits `NOT_BIS`. The displayed author is the source RC awarder when that award resolves; the stored outcome author stays the Spectrum writer. RCLC history with `responseID == "BONUS_ROLL"` is a separate external `BONUS_ROLL` event and does not create `RC_LOOT_COUNCIL` or an automatic BiS row. A legacy `RC_LOOT_COUNCIL` row with that `responseId`, and any `BIS_OUTCOME` sourced from it, is ignored by replay so it does not consume a slot. Gear Override legacy association uses the projected `displayedSlot` when it exists. Live RC integration edits do not push a full `PROFILE_SNAPSHOT`. An ordinary admin proposes `RC_CONFIG_REQ` (RC integration fields only) to the coordinator; the coordinator serializes a monotonic session `seq`, stamps accepted `(rcConfigEpoch, seq)`, and RAID-broadcasts `RC_CONFIG_SET`. Live SET keeps `coordEpoch` as the live session epoch so control admission still works; catch-up replay restates the stored accepted generation instead of rewriting it as `(liveCoordEpoch, seq)`. Peers apply a SET when that accepted generation is newer, or when they must converge an unpublished/dirty equal generation. A non-coordinator edit remains a proposal until that SET; award-time recording, BiS qualification, advertised descriptors, and helper snapshots use accepted configuration only. Unaccepted in-session proposals are discarded when that session ends and do not become `_rcConfigDirty` drafts. Out-of-session edits on an already-accepted generation stay unpublished so they cannot impersonate that generation; the next starter's accepted blob is canonical unless that starter minted their own unpublished edit. Full snapshots stay coordinator/helper-trusted (`NEED_PROFILE` / `PROFILE_SNAPSHOT`) and still carry `rcLootCouncilIntegration`, `rcConfigSeq`, and `rcConfigEpoch` for joiners.
...
Events are `CONSUMABLE_DONATION`, `CONSUMABLE_CRAFTER_RECEIPT`, `CONSUMABLE_CUSTODY`, `CONSUMABLE_CUSTODY_RESOLVE`, and `CONSUMABLE_CONFIG_RESET`. One client writes each operation: the receiving client for a direct trade, the depositor for a guild bank deposit, the withdrawing character for a withdrawal, and the resolving admin for custody resolve. Replay is idempotent by event id. Custody projection prefers a coordinator-assigned `order` when both events have one, so a skewed timestamp cannot apply a transfer before the withdrawal that created the stack. Snapshot import merges `snapshot.consumables` from `ImportSnapshot`.

`ConsumablesRouting.lua` and `ConsumablesWorkflow.lua` are pure. Direct routing treats a peer as compatible only when that peer advertised `consumablesCapable`; a negotiated protocol or addon version is not enough. `ConsumablesSync.lua` authorizes remote config and events, including the custody action and the writer allowed to record it. `modules/LootHelperSync/19_Consumables.lua` carries `CONSUMABLES_OP`, `CONSUMABLES_CONFIG`, and `CONSUMABLES_EVENT` on the existing bulk channel. Config broadcasts are accepted only from the session coordinator. Event and config payloads must carry the current session id and come from a member who is in the group. Protocol version stays 4. Older clients ignore the new message types and the optional capability field. Snapshots gain an optional `consumables` field; the snapshot version stays 1. Safe mode queues consumables events and flushes a bounded batch once bulk traffic is allowed again. A fingerprint mismatch also requests one snapshot and resends unsequenced local events in bounded batches.
...
Retail trade completion is inferred from both sides accepting and then `TRADE_CLOSED`. There is no completion event. The runtime must not accept a trade. Slot filling uses container pickup and `ClickTradeButton` from the trade-open handler. An open trade or deposit commits on the profile captured when it started. Guild bank identity uses `C_Club.GetGuildClubId()`. A withdrawal is recorded only after this client picks up a requested stack from the configured tab; a tab decrease without that pickup is ignored. Mobile Banking is resolved by spell name at runtime and is omitted when that spell is unknown. Its secure button is shown and configured only out of combat.
