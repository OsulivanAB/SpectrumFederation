...
The current protocol version is **2**. Clients on protocol 1 cannot participate in a linked-identity session; mixed Main Swap and Linked Character interpretation of the same profile is unsafe. `PROTO_MIN`, `PROTO_MAX`, `PROTO_CURRENT`, and `Sync.PROTO_VERSION` must stay aligned. Incoming `PROTO_NACK` and local unsupported-protocol chat warnings print once per peer and incompatibility signature until reload; repeats stay in debug logs.
